...
class Perceptron(Learner):
    ...
    def train_weighted(self, observations, labels, weights):
        for i,_ in enumerate(observations):
            self.train_online(observations[i], labels[i], weights[i], ret=False)
        return self.average_weights[:] if self.average else self.weights[:]

    def train_epochs_weighted(self, observations, labels, weights, epochs):
        zipped = zip(observations, labels, weights)
        for _ in range(epochs):
            random.shuffle(zipped)
            yield self.train_weighted(*zip(*zipped))
    ...
    def _update_weights(self, observation, label, weight=None):
        # Learning rate priority: `weight` > aggressive > dynamic_learning > default
        if weight:
        ...
        if self.aggressive:
            ...
            learning_rate = self.learning_rate / float(1 + self.update_count)
        else:
